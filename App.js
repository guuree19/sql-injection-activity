...
app.post('/signup', function (req, res) {
    ...
    var query = "SELECT title FROM user where username = '" + username + "'";
    ...
    db.get(query, function (err, row) {
        if (err) {
            ...
            res.redirect("/signup.html#usernameexists");
        } else {
            var title = 'User';
            db.run("INSERT INTO user VALUES ('" + username + "', '" + password + "', '" + title + "')", function (err) {
                if (err) {
                    console.log('ERROR', err);
                    res.redirect("/signup.html#error");
                } else {
